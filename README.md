...
- 可用滚动框设定上班、下班时间；在设定时刻准点全屏询问是否开始上班或下班。
...
- 可在主界面启用“开机自动启动”。
- 上班状态和下一次提醒时间会持久保存，程序重启后继续原有工作周期。
- 程序启动时会在后台自动补齐当年和下一年的中国大陆法定节假日日历。
...
- Set start and end times with hour/minute spin controls; full-screen confirmation appears during the exact scheduled minute.
...
- Work status and the next reminder time persist across application restarts.
- On startup, the app automatically refreshes missing calendar data for the current and following year.
